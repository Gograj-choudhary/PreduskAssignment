...
export const ProjectAdd = () => {
  const { mutate, isPending, isError, error } = useProject(AddProject);
  ...
  const handleAddProject = (e) => {
    ...
    // Convert skillsUsed into array
    if (data.skillsUsed) {
      data.skillsUsed = data.skillsUsed
        .split("\n")
        .map((line) => line.trim())
        .filter((line) => line.length > 0);
    }
  ...
  return (
    <div className="min-h-screen bg-[#FFFDD0] text-[#1A1800] flex items-center justify-center p-8">
      <div className="w-full max-w-3xl bg-[#FFFBA5] shadow-md rounded-xl p-8">
        <h1 className="text-3xl font-bold mb-8 text-center">Add Project</h1>
